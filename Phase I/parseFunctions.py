...
def parseI(instruction, line_number, table):
    ...
    immediate = ''
    try:
        [operation, rs, rd, immediate] = instruction.split()
    except:
        [operation, rs, rd] = instruction.split()
    #Check error code for operation
    if(operation not in I):
        raise Exception("The entered instruction is not a valid operation >>" + instruction)
        return None
    ...
    if(register == None or len(register.group()) != len(rs)):
        raise Exception("Enter a valid source register >>"+instruction)
    #Checking for rd
    if(operation in I[3:]):
        regex = re.compile("(\d+\(x\d+\))|(0x(\d+|[A-Fa-f]+)\(x\d+\))")
        register = regex.search(rd)
        if(register == None or len(register.group()) != len(rd)):
            raise Exception("Enter a valid destination register and its offset >>" + instruction)
        fill_imm = True
        rd = ''
    ...
    #Checking for rd
        ...
        register = regex.search(rd)
        if(register == None or len(register.group()) != len(rd)):
            raise Exception("Enter a valid destination register >>"+instruction)    
    #Get the integer based value of registers
    rs = int(rs[1:])
    rd = int(rd[1:])
    #Exception Handling
    if(rs < 0 or rs > 31 or rd < 0 or rd > 31):
        raise Exception("Invalid register value!!! They should be between 0 to 31 >>"+instruction)
    #Encoding to binary
    rs = str(bin(rs))
    rs = rs[2:]
    rs = '0' * (5 - len(rs)) + rs
    ...
    try:
        ...
        raise Exception('Enter a valid immediate field >>' + instruction)
    if(immediate[0:2] == '0x'):
    ...
    if(immediate > 2047 or immediate < -2048):
        raise Exception('Immediate value should be in the range [-2048, 2047] >>' + instruction)
    if(immediate < 0):
